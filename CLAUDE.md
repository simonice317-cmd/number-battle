...
- **技术栈**：原生 JS (ES Modules) + CSS 夜墨竞技深色主题 + PeerJS WebRTC
...
├── css/style.css       # 夜墨竞技深色主题（Premium Off-Black Dark），CSS 自定义属性驱动
├── js/
...
| ID | 名 | HP | 特色技能 | 组合技 |
|----|-----|----|----------|--------|
| basic | 基础使者 | 4 | 冲拳(4,8→伤1)、护盾(5→+1盾)、回血(9→+1HP) | 无 |
| paladin | 圣骑士 | 5 | 冲拳(4,8)、盾击(5→盾+伤)、号角(6→攻+1)、回血(9) | 5+5→圣光复苏(恢复上限) |
| archer | 弓箭手 | 3 | 速射(3,4,8→伤1)、轻甲(5→+1盾)、灵药(9→回1) | 3+6→万箭齐发(伤2+回1) |
| thief | 盗贼 | 2 | 刺击(4,8→伤1)、妙手(6→偷资源)、重击(7→穿甲)、暗影斗篷(9→+1盾) | 8+7→暗影突袭(破盾+伤1) |
...
| ID | 名称 | 图标 | 效果 |
|----|------|------|------|
| `heal_2` | 生命恢复 | 疗 | 恢复 2 点 HP（不超过上限） |
| `restore_max` | 上限修复 | 复 | 恢复 2 点 maxHp（不超过 baseMaxHp） |
| `hp_lock` | 绝地求生 | 锁 | HP/maxHp 不低于 1，持续到己方下回合开始 |
